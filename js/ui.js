...
const UI = {
    ...
    // Render Think view - displays Claude's analysis
    renderThinkView(options) {
        let html = '';
                    ...
                    <div class="think-dialog">
                        <div class="think-messages" id="thinkMessages">
                            <div class="think-message assistant">
                                ...
                                <div class="message-content">${this.formatAnalysisContent(options.analysis)}</div>
                            </div>
                        </div>
                        ...
                        <div class="think-input-container">
                            <textarea 
                                ...
                                rows="2"
                                onkeydown="if(event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); EventHandler.handleThinkQuestion(this.value); }"
                            ></textarea>
                            <button class="btn btn-primary" onclick="EventHandler.handleThinkQuestion(document.getElementById('thinkQuestion').value)">
    ...
    // Update Think dialog with new messages
    updateThinkDialog(options) {
                    ...
                    if (msg.role !== 'system') {
                        const msgHtml = `
                            <div class="think-message ${msg.role}">
                                ...
                                <div class="message-content">${this.formatAnalysisContent(msg.content)}</div>
                            </div>
                        `;
    ...
    // Format analysis content with line breaks
